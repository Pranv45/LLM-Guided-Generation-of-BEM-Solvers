...
def precompute_quadrature(nodes, elems, areas):
    Ne = elems.shape[0]
    quad_y = np.zeros((Ne, 3, 3), dtype=np.float64)
    quad_wJ = np.zeros((Ne, 3), dtype=np.float64)
    tri_phi = np.array([[2/3, 1/6, 1/6], [1/6, 2/3, 1/6], [1/6, 1/6, 2/3]], dtype=np.float64)

    for i in range(Ne):
        p0, p1, p2 = nodes[elems[i, 0]], nodes[elems[i, 1]], nodes[elems[i, 2]]
        for q in range(3):
            quad_y[i, q] = tri_phi[0, q]*p0 + tri_phi[1, q]*p1 + tri_phi[2, q]*p2
            quad_wJ[i, q] = areas[i] / 3.0
    return quad_y, quad_wJ, tri_phi

# ==========================================
# NUMBA ACCELERATED BEM ASSEMBLY
# ==========================================
@njit(parallel=True, fastmath=True)
def assemble_system(nodes, elems, areas, normals, quad_y, quad_wJ, tri_phi, node_elems, node_elems_count, gauss_pts, gauss_wts):
    Nn = nodes.shape[0]
    Ne = elems.shape[0]
    ...
    for i_node in prange(Nn):
        for e_idx in range(node_elems_count[i_node]):
            ...
            for x_q in range(3):
                x0 = quad_y[e_outer, x_q, 0]
                x1 = quad_y[e_outer, x_q, 1]
                x2 = quad_y[e_outer, x_q, 2]
                Wx = quad_wJ[e_outer, x_q] * tri_phi[loc_i, x_q]

                L0_x, L1_x, L2_x = tri_phi[0, x_q], tri_phi[1, x_q], tri_phi[2, x_q]
                ...
                for e_inner in range(Ne):
                    ...
                    if e_outer == e_inner:
                        ...
                        for y_q in range(3):
                            y0 = quad_y[e_inner, y_q, 0]
                            y1 = quad_y[e_inner, y_q, 1]
                            y2 = quad_y[e_inner, y_q, 2]
                            Wy = quad_wJ[e_inner, y_q]
                            ...
                            if r_sq > 1e-28:
                                ...
                                p0, p1, p2 = tri_phi[0, y_q], tri_phi[1, y_q], tri_phi[2, y_q]
...
# ==========================================
@njit(parallel=True, fastmath=True)
def eval_interior(pts, nodes, elems, u, q, quad_y, quad_wJ, normals, tri_phi):
    N_pts = pts.shape[0]
    Ne = elems.shape[0]
    ...
    for i in prange(N_pts):
        ...
        for e in range(Ne):
            ...
            for y_q in range(3):
                ...
                phi0, phi1, phi2 = tri_phi[0, y_q], tri_phi[1, y_q], tri_phi[2, y_q]
...
def solve_bem(N):
    ...
    quad_y, quad_wJ, tri_phi = precompute_quadrature(nodes, elems, areas)
    ...
    H, G = assemble_system(nodes, elems, areas, normals, quad_y, quad_wJ, tri_phi, node_elems, node_elems_count, gauss_pts, gauss_wts)
    ...
    u_eval = eval_interior(eval_pts, nodes, elems, u, q, quad_y, quad_wJ, normals, tri_phi)
...
def main():
    ...
    for N in Ns:
        ...
        print(f"{N:<4} | {Ne:<7} | {err:.6e}   | {ts:.4f}    | {tsol:.4f}    | {te:.4f}   | {tt:.4f}")
        results.append(err)
