# Rectangle BEM solver (single Jupyter notebook cell)
# Methodology mirrors the provided annulus reference implementation:
# - Direct BEM (Galerkin) with single-layer G and double-layer H
# - 2-node linear panels, special treatment of singular integrals
# - 16-point Gauss + transformed log-split quadrature for singular part
# - Dense Galerkin assembly (compute_galerkin_matrices) accelerated with numba
# - Assemble linear system using same column-sign convention as reference
# - Solve with GMRES (matrix applied via dense A@v)
# - Interior evaluation and relative L2 error computation
import numpy as np
import time
...
def exact_q(x, y, nx, ny):
    ...
    uy = (np.sinh(np.pi * x) / np.sinh(2.0 * np.pi)) * (-np.pi * np.sin(np.pi * y))
    return ux * nx + uy * ny
...
def compute_galerkin_matrices(nodes_x, nodes_y, elems, nx, ny, lengths, pts_gl, wts_gl, pts_01, wts_01, pts_log, wts_log):
    ...
    for et in prange(N_el):
        ...
        for es in range(N_el):
            ...
            if et == es:
                # self-element: handle singular pieces semi-analytically (same pattern as reference)
                mass11 = Lt / 3.0
                mass12 = Lt / 6.0
                mass22 = Lt / 3.0
                h11 += 0.5 * mass11
                ...
                for kt in range(len(pts_gl)):
                    xt = pts_gl[kt]; wt = wts_gl[kt]
                    Nt1 = 0.5 * (1.0 - xt); Nt2 = 0.5 * (1.0 + xt)

                    pt_x = Nt1 * nodes_x[it1] + Nt2 * nodes_x[it2]
                    pt_y = Nt1 * nodes_y[it1] + Nt2 * nodes_y[it2]

                    for ks in range(len(pts_gl)):
                        xs = pts_gl[ks]; ws = wts_gl[ks]
                        Ns1 = 0.5 * (1.0 - xs); Ns2 = 0.5 * (1.0 + xs)
...
def solve_bem(N_side):
    t_start = time.time()

    # number of panels per side -> total nodes / panels = 4 * N_side
    N_tot = 4 * N_side
    ...
    # Node ordering (counter-clockwise around rectangle) starting at (0,0):
    # bottom side: (0,0) -> (2,0)
    idx = 0
    for i in range(N_side):
        t = i / N_side
        nodes_x[idx] = 0.0 + t * 2.0
        nodes_y[idx] = 0.0
        elems[idx, 0] = idx
    ...
    for i in range(N_side):
        t = i / N_side
        nodes_x[idx] = 0.0
        nodes_y[idx] = 1.0 - t * 1.0
        elems[idx, 0] = idx
        elems[idx, 1] = (idx + 1) % N_tot
        idx += 1

    # element geometry and outward normals
    for e in range(N_tot):
        j1 = elems[e, 0]; j2 = elems[e, 1]
        dx = nodes_x[j2] - nodes_x[j1]; dy = nodes_y[j2] - nodes_y[j1]
        L = np.hypot(dx, dy)
        lengths[e] = L
        # For the chosen CCW ordering, outward normal = (dy/L, -dx/L)
        nx[e] = dy / L
        ny[e] = -dx / L

    # get quadrature templates
    pts_gl, wts_gl, pts_01, wts_01, pts_log, wts_log = get_gauss_points()

    # assemble Galerkin matrices (dense)
    ...
    is_u_known = np.zeros(N_tot, dtype=np.bool_)

    # assign BCs:
    # Dirichlet: left side x=0 -> u=0 ; right side x=2 -> u=cos(pi y)
    # Neumann: bottom y=0 and top y=1 -> du/dy = 0  (i.e., q = exact_q using outward normal)
    for i in range(N_tot):
        ...
        if abs(x - 0.0) < 1e-12:
            ...
            is_u_known[i] = True
            u_known[i] = np.cos(np.pi * y)
        elif abs(y - 0.0) < 1e-12 or abs(y - 1.0) < 1e-12:
            # bottom or top: Neumann q = exact_q with outward normal (average of adjacent element normals)
            is_u_known[i] = False
            e_curr = i
            e_prev = (i - 1) % N_tot
            nx_avg = nx[e_prev] + nx[e_curr]
            ny_avg = ny[e_prev] + ny[e_curr]
            norm = np.hypot(nx_avg, ny_avg)
            if norm == 0.0:
                nx_avg = nx[e_curr]; ny_avg = ny[e_curr]; norm = np.hypot(nx_avg, ny_avg)
            nx_avg /= norm; ny_avg /= norm
            q_known[i] = exact_q(x, y, nx_avg, ny_avg)
        else:
            # fallback (shouldn't happen)
            is_u_known[i] = True
            u_known[i] = exact_u(x, y)

    # build linear system A x = b using same column-sign convention as reference
    A = np.zeros((N_tot, N_tot))
    b = np.zeros(N_tot)

    for j in range(N_tot):
        if is_u_known[j]:
            # column corresponds to known u: put -G[:,j], move H[:,j]*u_known to RHS with minus sign
            A[:, j] = -G[:, j]
            b -= H[:, j] * u_known[j]
        else:
            # column corresponds to known q: put H[:,j], move G[:,j]*q_known to RHS with plus sign
            A[:, j] = H[:, j]
            b += G[:, j] * q_known[j]

    t_setup = time.time() - t_start

    # solve using GMRES (matrix-free matvec via A@v)
    ...
    # reconstruct full boundary arrays
    ...
    # evaluate interior solution on a grid inside the rectangle
...
if __name__ == "__main__":
    ...
    # refinement study requested (N = 10,20,40,80)
    N_values = [40, 80, 160, 320, 640]
    results = []

    print(f"{'N':<4} {'Unknowns':<10} {'GMRES':<7} {'Rel L2 Error':<13} {'Setup':<9} {'Solve':<9} {'Eval':<9} {'Total':<9}")
    print("-" * 90)

    for n in N_values:
        res = solve_bem(n)
        results.append(res)
        print(f"{res['N']:<4d} {res['Unknowns']:<10d} {res['GMRES']:<7d} {res['Error']:13.6e} {res['Setup']:<9.3f} {res['Solve']:<9.3f} {res['Eval']:<9.3f} {res['Total']:<9.3f}")
